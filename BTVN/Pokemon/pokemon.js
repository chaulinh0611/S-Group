...
async function display(pokemonSet) {  
    ...
    for (const pokemon of pokemonSet) {
        ...
        const pokemon_number = pokemonData.id; 
        const image = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemon_number}.png`;
        const labels = pokemonData.types.map(type => 
            `<span class="label ${type.type.name}">${type.type.name}</span>`
        ).join(" ");
        ...
        pokemon_card.innerHTML = `
            <a class="card" href="">
                <p class="pokemon-number">#${pokemon_number}</p>
